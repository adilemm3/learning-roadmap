...
### Как работает
1. Пишешь `учим` или `продолжить` → Claude загружает контекст и ведёт по плану
2. Каждая тема разбита на блоки: теория → проверка → практика
...
### Дополнительные файлы для Learn Mode
- `learner-profile.md` — профиль ученика (стиль обучения, предпочтения)
- `templates/topic/` — шаблоны файлов для новых тем (9 файлов)
- `templates/section/` — шаблоны для родительских узлов (3 файла)
- Каждая подтема получает два дополнительных файла: `learning-plan.md` и `session.md`

### Скиллы автоматизации
Создай директорию `.claude/skills/` с пятью скиллами:
- `learn/SKILL.md` — запуск и ведение обучения (startup protocol, цикл, обновление файлов)
- `save-session/SKILL.md` — чеклист сохранения прогресса (session, plan, summary, progress, weak-spots, profile)
- `create-topic/SKILL.md` — создание новой темы с файлами и обновлением мета-файлов
- `rebalance/SKILL.md` — разбиение большой темы на подтемы
- `assess/SKILL.md` — оценка уровня через вопросы

Содержимое скиллов — см. дизайн-документ `docs/designs/2026-03-05-learn-mode-design.md`.
